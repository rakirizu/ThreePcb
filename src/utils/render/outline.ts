...
export const renderImageOutline = (element: ImageGraphic, shape: THREE.Shape) => {
    if (element.type != 'imagePath') {
        console.warn('[Three-PCB] Invalid outline type', element)
        return null
    }
    if (element.segments.length != 1) {
        console.warn('[Three-PCB] Invalid outline segments length', element)
        return null
    }
    ...
    if (path.type == ARC) {
        ...
        console.warn('[Three-PCB] Invalid outline type', path)
    }
}
