...
function contourizeCirclePath(segment: Tree.PathSegment, width: number): Tree.PathSegment[] {
    const { start, end } = segment
    if (segment.type === Tree.PLOT_LINE) {
        const [x1, y1] = start
        const [x2, y2] = end
...
function createArcSegment(
    ...
    const [start, end, center] = getArcPositions(startPoint, endPoint, centerPoint, arcDirection)
