...
import type { MacroPrimitiveCode, MacroValue } from '../../parser'
import {
    MACRO_CENTER_LINE,
    MACRO_CIRCLE,
...
import { PI, positionsEqual, rotateAndShift } from '../coordinate-math'

import type { Location } from '../location-store'
import type { MacroTool } from '../tool-store'
import * as Tree from '../tree'

import { CCW, CW, getArcPositions } from './plot-path'

type VariableValues = Record<string, number>

export function plotMacro(tool: MacroTool, location: Location): Tree.LayeredShape {
...
function plotCircle(origin: Tree.Position, parameters: number[]): Tree.ErasableShape {
    const [exposure, diameter, cx0, cy0, degrees] = parameters
    const r = diameter / 2
    const [cx, cy] = rotateAndShift([cx0, cy0], origin, degrees)

    return { type: Tree.PLOT_CIRCLE, erase: exposure === 0, cx, cy, r }
}

function plotVectorLine(origin: Tree.Position, parameters: number[]): Tree.ErasableShape {
    const [exposure, width, sx, sy, ex, ey, degrees] = parameters
    const [dy, dx] = [ey - sy, ex - sx]
    const halfWidth = width / 2
    const distance = Math.sqrt(dy ** 2 + dx ** 2)
    const [xOff, yOff] = [(halfWidth * dy) / distance, (halfWidth * dx) / distance]
...
function plotCenterLine(origin: Tree.Position, parameters: number[]): Tree.ErasableShape {
...
function plotLowerLeftLine(origin: Tree.Position, parameters: number[]): Tree.ErasableShape {
...
function plotOutline(origin: Tree.Position, parameters: number[]): Tree.ErasableShape {
...
function plotPolygon(origin: Tree.Position, parameters: number[]): Tree.ErasableShape {
    ...
    return { type: Tree.PLOT_POLYGON, erase: exposure === 0, points }
}

function plotMoire(origin: Tree.Position, parameters: number[]): Tree.ErasableShape[] {
    const rotate = (p: Tree.Position): Tree.Position => rotateAndShift(p, origin, parameters[8])
    ...
    return [
        ...radii.flatMap(({ r, erase }) => {
            return { type: Tree.PLOT_CIRCLE, cx, cy, r, erase } as Tree.CircleShape
        }),
...
function plotThermal(origin: Tree.Position, parameters: number[]): Tree.ErasableShape {
    ...
    for (const rot of positions) {
        ...
        segments.push(
            { type: Tree.PLOT_LINE, start: points[0], end: points[1] },
            { type: Tree.ARC, start: os, end: oe, center: oc, radius: or },
            { type: Tree.PLOT_LINE, start: points[2], end: points[3] }
        )
