...
import type { ArcPosition, SizeEnvelope as Box, Position } from './tree'
...
export function fromGraphic(graphic: Tree.ImageGraphicBase): Box {
    return graphic.type === Tree.IMAGE_SHAPE
        ...
        : fromPath(graphic.segments, graphic.type === Tree.IMAGE_PATH ? graphic.width : undefined)
}

export function fromShape(shape: Tree.Shape): Box {
    switch (shape.type) {
        case Tree.PLOT_CIRCLE: {
            const { cx, cy, r } = shape
            return fromPosition([cx, cy], r)
        }

        case Tree.PLOT_RECTANGLE: {
            const { x, y, xSize, ySize } = shape
            return [x, y, x + xSize, y + ySize]
        }

        case Tree.PLOT_POLYGON: {
        ...
        case Tree.LAYERED_SHAPE: {
            return sum(shape.shapes.filter(({ erase }) => erase !== true).map(fromShape))
        }
    }
}
...
export function fromPath(segments: Tree.PathSegment[], width = 0): Box {
    ...
    for (const segment of segments) {
        ...
        if (segment.type === Tree.ARC) {
            ...
            let [thetaStart, thetaEnd] = end[2] > start[2] ? [start[2], end[2]] : [end[2], start[2]]
