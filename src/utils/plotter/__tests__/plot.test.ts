...
import type { Location, LocationStore } from '../location-store'
import type { PlotOptions } from '../options'
import type { Tool, ToolStore } from '../tool-store'
...
describe('creating a plot tree', () => {
    ...
    beforeEach(async () => {
        ...
        td.when(createGraphicPlotter(Parser.GERBER), { times: 1 }).thenReturn(graphicPlotter)
    })
    ...
    it('should get plot options and plot', () => {
        ...
        const shape2: Tree.ImageGraphic = {
            type: Tree.IMAGE_SHAPE,
            shape: { type: Tree.PLOT_CIRCLE, cx: 4, cy: 5, r: 6 },
