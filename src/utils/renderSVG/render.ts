...
export function shapeToElement(shape: Shape): SvgElement {
    switch (shape.type) {
        ...
        case PLOT_POLYGON: {
            const points = shape.points.map(([x, y]: number[]) => `${x},${-y}`).join(' ')
...
export function renderPath(node: ImagePath | ImageRegion): SvgElement {
    ...
    const props = node.type === IMAGE_PATH ? { strokeWidth: node.width, fill: 'none' } : {}
